/**
 * Backtest WR par bande de change_pct (1min) pour une asset_class — répond
 * "à partir de quel niveau de pump le chase-the-top devient perdant ?".
 * Simule TP/SL configurable sur horizon, marché réel (EODHD candles 5m).
 *
 * Usage : EODHD_API_KEY=xxx npx tsx scripts/backtest-overextended-by-band.ts \
 *           --market=us_equity_small_mid --tp=3 --sl=1.5 --horizon=60
 *
 * Résultat 03/06 (us_equity_small_mid, n=2444) :
 *   3-8%   : WR 43.2% exp +0.417%
 *   8-10%  : WR 55.6% exp +0.967%  ← sweet spot
 *   10-15% : WR 34.3% exp +0.043%  ← breakeven, rejet justifié
 *   15-25% : WR 26.7% exp -0.283%  ← perdant
 * → seuil GAINERS_MAX_CHANGE_PCT_LONG_US_SMALL_MID optimal = 10.
 */
...
const MARKET=arg('market','us_equity_small_mid'), HORIZON=+arg('horizon','60'), TP=+arg('tp','3'), SL=+arg('sl','1.5');
const cache=new Map<string,any[]>();
async function candles(sym:string,from:number,to:number){
  const k=sym+'::'+Math.floor(from/86400)+'::'+Math.floor(to/86400);
...
function outcome(post:any[],entry:number){
  const tp=entry*(1+TP/100),sl=entry*(1-SL/100);
  for(const c of post){if(c.low<=sl)return 'LOSE';if(c.high>=tp)return 'WIN';}
  const last=post[post.length-1].close;
  if(last>=entry*1.002)return 'WIN';if(last<=entry*0.998)return 'LOSE';return 'NEUTRAL';
}
const BANDS=[[3,8],[8,10],[10,15],[15,25],[25,100]];
(async()=>{
  const rows:any[]=[];
  for(let d=0;d<28;d++){
    const f=new Date(Date.now()-(d+1)*86400_000).toISOString(),t=new Date(Date.now()-d*86400_000).toISOString();
    const {data}=await sb.from('top_gainers_log').select('symbol,captured_at,close_price,change_pct,market').eq('market',MARKET).gte('captured_at',f).lt('captured_at',t).limit(800);
    if(data)rows.push(...data);
  }
  console.log(MARKET+' candidates 28d:',rows.length,'TP+'+TP+'/SL-'+SL+' h'+HORIZON);
  for(let i=rows.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[rows[i],rows[j]]=[rows[j],rows[i]];}
  const per:any={};const samp:any[]=[];
  for(const r of rows){if(samp.length>=4000)break;const n=per[r.symbol]??0;if(n>=25)continue;per[r.symbol]=n+1;samp.push(r);}

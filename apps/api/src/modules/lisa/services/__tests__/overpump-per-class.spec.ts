...
describe('resolveOverpumpThreshold', () => {
  ...
  it('eu_equity → 15 par default', () => {
    expect(resolveOverpumpThreshold('eu_equity', DEFAULT_OVERPUMP_PER_CLASS, 0)).toBe(15);
  });
  it('us_equity_large → 15', () => {
    expect(resolveOverpumpThreshold('us_equity_large', DEFAULT_OVERPUMP_PER_CLASS, 0)).toBe(15);
  });
  it('us_equity_small_mid → 15', () => {
    expect(resolveOverpumpThreshold('us_equity_small_mid', DEFAULT_OVERPUMP_PER_CLASS, 0)).toBe(15);
  });
  ...
  it('class inconnue → conservative 15 (us_large)', () => {
    expect(resolveOverpumpThreshold('xyz', DEFAULT_OVERPUMP_PER_CLASS, 0)).toBe(15);
  ...
  it('globalOverride > 0 ET >= per-class → per-class (override ne resserre rien)', () => {
    expect(resolveOverpumpThreshold('eu_equity', DEFAULT_OVERPUMP_PER_CLASS, 20)).toBe(15);
  });
...
describe('describeOverpumpOverrides', () => {
  it('descriptif lisible', () => {
    expect(describeOverpumpOverrides(DEFAULT_OVERPUMP_PER_CLASS))
      .toBe('asia=30 eu=15 us_large=15 us_small_mid=15 crypto=30');
  });
});
